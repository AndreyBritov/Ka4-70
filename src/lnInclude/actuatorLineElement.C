...
Foam::scalar Foam::fv::actuatorLineElement::inflowRefAngle()
{
    ...
    //Info <<(acos(float((-relativeVelocity_ & chordRefDirection_)/(mag(relativeVelocity_) * mag(chordRefDirection_))))) << endl;
    scalar inflowVelAngleRad = Foam::acos(float
    (
        (-relativeVelocity_ & chordRefDirection_)
        / (mag(relativeVelocity_) * mag(chordRefDirection_))
    ));
//	if (inflowVelAngleRad == 0)
//    {inflowVelAngleRad = 0.0872665;}
...
)
{
    ...
    scalar angleOfAttackRad;
    if (mag(relativeVelocity_) == 0)
    {angleOfAttackRad = 0;}
    else
    {
        angleOfAttackRad = asin((planformNormal_ & relativeVelocity_)
                            / (mag(planformNormal_)
    ...
    if (mag(relativeVelocityGeom_) == 0)
    {angleOfAttackGeom_ = 0;}
    else
    {
    ...
    if (mag(liftDirection) == 0)
    {
        liftDirection = vector(0, 0, 0);
    }
    else
    {
